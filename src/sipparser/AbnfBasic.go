...
func SipUserNeedEscape(src []byte) bool {
	for i := 0; i < len(src); i++ {
		if !IsSipUser(src[i]) {
			return true
		}
...
func Escape(src []byte, inCharset AbnfIsInCharset) (dst []byte) {
	...
	for i := 0; i < len(src); i++ {
		if inCharset(src[i]) {
			...
			dst = append(dst, '%', ToUpperHex(src[i]>>4), ToUpperHex(src[i]))
		}
	}
...
func WriteSipUserEscape(buf *AbnfByteBuffer, src []byte) {
	for i := 0; i < len(src); i++ {
		if IsSipUser(src[i]) {
			buf.WriteByte(src[i])
		} else {
			buf.WriteByte('%')
			buf.WriteByte(ToUpperHex(src[i] >> 4))
			buf.WriteByte(ToUpperHex(src[i]))
		}
	}
}

func WriteSipPnameEscape(buf *AbnfByteBuffer, src []byte) {
	for i := 0; i < len(src); i++ {
		if IsSipPname(src[i]) {
			buf.WriteByte(src[i])
		} else {
